...
export default function EditorPage() {
  ...
  // Auto-generate outline if generate=true parameter is present and slides are empty
  useEffect(() => {
    if (typeof window !== "undefined") {
      const searchParams = new URLSearchParams(window.location.search);
      const shouldGenerate = searchParams.get("generate") === "true";
      if (shouldGenerate && deck && slides.length === 0 && !generateStarted) {
        setGenerateStarted(true);
        runGenerateOutline({ deckId: id as any, prompt: deck.title }).then(() => {
          // Clear query params
          window.history.replaceState({}, "", `/deck/${id}/editor`);
        });
      }
    }
  }, [deck, slides.length, generateStarted]);
  ...
  return (
    <div className="h-screen flex flex-col bg-background">
      <div className="flex items-center justify-between px-4 py-2 border-b border-divider bg-content1/80">
        ...
        <div className="flex items-center gap-2">
          <Button isIconOnly variant="light" size="sm">
            <Undo2 className="w-4 h-4 text-default-400" />
          </Button>
          <Button isIconOnly variant="light" size="sm">
            <Redo2 className="w-4 h-4 text-default-400" />
          </Button>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-64 bg-content1 border-r border-divider flex flex-col">
          <div className="p-3 border-b border-divider flex items-center justify-between">
            ...
            <Button isIconOnly size="sm" variant="light" onPress={handleAddSlide}>
              <Plus className="w-3.5 h-3.5 text-default-400" />
            </Button>
          </div>
              ...
              return (
                <div key={slide._id} onClick={() => setSelectedSlide(idx + 1)} className={`relative p-2.5 rounded-xl cursor-pointer transition-all duration-200 border ${isActive ? "bg-[#7170FF]/15 border-[#7170FF]/40 shadow-sm" : "bg-[#0F1011] border-white/[0.06] hover:border-white/[0.15]"}`}>
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="w-9 h-6 bg-[#151617] rounded border border-white/[0.05] flex items-center justify-center">
                      <Icon className="w-3.5 h-3.5 text-default-400" />
                    </div>
        ...
        <div className="flex-1 flex flex-col bg-background">
          <div className="flex-1 flex items-center justify-center p-8 bg-[#08090A]">
            <div ref={canvasRef} className="w-full max-w-4xl aspect-video bg-[#0F1011] rounded-2xl border border-white/[0.08] shadow-2xl p-12 relative overflow-hidden flex flex-col justify-center">
              ...
              <div className="absolute bottom-6 right-6 text-xs text-default-400 font-mono tracking-wider">
              ...
              <div className="relative z-10 w-full">
                {slides.length === 0 ? (
                  <div className="text-center">
                    <Wand2 className="w-12 h-12 text-[#7170FF] mx-auto mb-4 animate-pulse" />
                    <p className="text-lg text-white font-medium">No slides in this deck</p>
                    <p className="text-sm text-default-400 mt-1">Generating presentation or add a slide to begin.</p>
                  </div>
                ) : (
                  <>
          ...
          <div className="px-8 py-3 border-t border-divider flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Button isIconOnly size="sm" variant="light" onPress={() => setSelectedSlide(Math.max(1, selectedSlide - 1))} disabled={slides.length === 0}>
                <ChevronLeft className="w-4 h-4 text-default-400" />
              </Button>
              <span className="text-sm text-default-400">{slides.length === 0 ? "0 / 0" : `${selectedSlide} / ${slides.length}`}</span>
              <Button isIconOnly size="sm" variant="light" onPress={() => setSelectedSlide(Math.min(slides.length, selectedSlide + 1))} disabled={slides.length === 0}>
                <ChevronRight className="w-4 h-4 text-default-400" />
              </Button>
            </div>
            <div className="flex items-center gap-2">
              <Button variant="flat" size="sm" startContent={<Monitor className="w-4 h-4" />} onPress={handlePresent} disabled={slides.length === 0}>Present</Button>
              <Button variant="flat" size="sm" startContent={<MessageSquare className="w-4 h-4" />} onPress={onOpen} disabled={slides.length === 0}>Notes</Button>
            </div>
          </div>
        </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-5 scrollbar-thin">
            <div>
              ...
              <div className="space-y-2">
                <Button size="sm" className="w-full bg-primary/20 text-primary border border-primary/30 font-semibold" startContent={<Sparkles className="w-4 h-4" />} onPress={handleRegenerate} isLoading={regenerating} disabled={!currentSlide || currentSlide.isLocked}>
                  Regenerate Slide
                </Button>
                
                <Dropdown>
                  <DropdownTrigger>
                    <Button size="sm" variant="flat" className="w-full" startContent={<Wand2 className="w-4 h-4" />} disabled={!currentSlide || currentSlide.isLocked}>Change Tone</Button>
                  </DropdownTrigger>
                ...
                <Dropdown>
                  <DropdownTrigger>
                    <Button size="sm" variant="flat" className="w-full" startContent={<Type className="w-4 h-4" />} disabled={!currentSlide || currentSlide.isLocked}>Expand / Shorten</Button>
                  </DropdownTrigger>
            ...
            <div>
              ...
              <div className="space-y-2">
                <Select label="Type" size="sm" defaultSelectedKeys={["chart"]} disabled={!currentSlide}>
                ...
                <Button size="sm" variant="flat" startContent={<Image className="w-4 h-4" />} disabled={!currentSlide}>Generate Image</Button>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                <Button size="sm" variant="flat" startContent={currentSlide?.isLocked ? <Lock className="w-4 h-4" /> : <Unlock className="w-4 h-4" />} onPress={handleToggleLock} disabled={!currentSlide}>
                  {currentSlide?.isLocked ? "Unlock Slide" : "Lock Slide"}
                </Button>
                <Button size="sm" variant="flat" startContent={<Copy className="w-4 h-4" />} onPress={handleDuplicate} disabled={!currentSlide}>
                  Duplicate
                </Button>
                <Button size="sm" variant="flat" className="text-danger" startContent={<Trash2 className="w-4 h-4" />} onPress={handleDelete} disabled={!currentSlide || slides.length <= 1}>
                  Delete Slide
                </Button>
              </div>
            </div>
      ...
      <Modal isOpen={isOpen} onClose={onClose} size="2xl">
        <ModalContent className="bg-surface-container-low text-foreground border border-border/50">
          ...
          <ModalBody>
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              <Button size="sm" variant="light" className="text-primary">Generate speaker notes</Button>
            </div>
          </ModalBody>
          <ModalFooter>
